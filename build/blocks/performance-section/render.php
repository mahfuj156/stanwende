...
<div class="<?= esc_attr($gridClass); ?> gap-10 md:gap-16 items-center">
...
<!-- IMAGE (mobile first → order 2, desktop → order 1) -->
    ...
    <div class="order-2 md:order-2 w-full">
      ...
      <?php if ($title): ?>
          <h2 class="text-h38 md:text-h46 font-bold leading-114 mb-4" style="font-size: <?= esc_attr($titleFontSize); ?>px;">
              <?= esc_html($title); ?>
          </h2>
      <?php endif; ?>

        <?php if ($subtitle): ?>
        <p class="text-info text-h18 font-bold leading-150 mb-4"  style="font-size: <?= esc_attr($subtitleFontSize); ?>px;"><?= wp_kses_post($subtitle); ?></p>
        <?php endif; ?>
    ...
    <?php endif; ?>
      ...
      <div class="flex flex-wrap gap-6 md:gap-8 items-start mb-8">
        <?php if($stat1 || $stat1_label): ?>
          <div>
              <?php if($stat1): ?><div class="text-h38 md:text-h46 font-bold leading-114 text-info"><?= esc_html($stat1); ?></div>  <?php endif; ?>
               <?php if($stat1_label): ?>    <div class="text-p16 text-secondary leading-150"><?= esc_html($stat1_label); ?></div>  <?php endif; ?>
          </div>
           <?php endif; ?>
            <?php if($stat2 || $stat2_label): ?>
            <div>
                <?php if($stat2): ?><div class="text-h38 md:text-h46 font-bold leading-114 text-info"><?= esc_html($stat2); ?></div>  <?php endif; ?>
                <?php if($stat2_label): ?><div class="text-p16 text-secondary leading-150"><?= esc_html($stat2_label); ?></div>  <?php endif; ?>
            </div>
            <?php endif; ?>
      </div>
